...
#Get help
#We can get help with '?' and the name of the function
?solve
...
#We can assign values for variables in three ways. The first two are normaly used whilet the other one is used for Global assingments
a = 5
a <- 5
...
#Get the class of the above varibles
class(a)
class(b)
...
gender <- c(1, 1, 0, 1)
gender
#We can get the value Male instead of 0 and Female instead of 1
